...
class OpenguassDatabaseConnector(DatabaseConnector):

    def __init__(self, db_name, autocommit=False):
        DatabaseConnector.__init__(self, db_name, autocommit=autocommit)
        self.db_system = "openguass"
        self._connection = None

        if not self.db_name:
            self.db_name = "openguass"
        self.create_connection()
        self.set_random_seed()
        self.exec_only("set enable_hypo_index = on;")
        logging.debug("openguass connector created: {}".format(db_name))

    def create_connection(self):
        if self._connection:
            self.close()
        user = input("please input user：\n")
        password = input("please input passwrd：\n")
        self.db_name = input("please input database：\n")
    ...
    def create_statistics(self):
        logging.info("openguass: Run `analyze`")
        self.commit()
    ...
    def set_random_seed(self, value=0.17):
        logging.info(f"openguass: Set random seed `SELECT setseed({value})`")
        self.exec_only(f"SELECT setseed({value})")

    def supports_index_simulation(self):
        if self.db_system == "openguass":
            return True
        return False
    ...
    def _drop_simulated_index(self, oid):
        ...
        assert result[
            0] is True, f"Could not drop simulated index with oid = {oid}."
    ...
    def drop_indexes(self):
        ...
        drop_stmt = "select * from hypopg_reset_index();"
