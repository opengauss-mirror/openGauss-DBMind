    ...
    def __init__(self,
        ...
        if retrieve_plans:
            for q_idx in range(1):
                ...
                self.plans = ([], [])
                for query_idx, query_texts_per_query_class in enumerate(
                        query_texts):
                    ...
                    while created_indexes < len(n_column_combinations):
                        ...
                        for i in range(self.INDEXES_SIMULATED_IN_PARALLEL):
                            potential_index = Index(
                                n_column_combinations[created_indexes])
                            cost_evaluation.what_if.simulate_index(
                                potential_index, True)
                        ...
                        for potential_index in potential_indexes:
                            cost_evaluation.what_if.drop_simulated_index(
                                potential_index)

                        logging.critical(
                            f"Finished checking {created_indexes} indexes of width {n+1}."
                        )
...
class SQLWorkloadEmbedder(WorkloadEmbedder):
    ...
    def __init__(self, query_texts, representation_size, database_connector,
                 columns):
        WorkloadEmbedder.__init__(self, query_texts, representation_size,
                                  database_connector)
        ...
        for query_idx, query_texts_per_query_class in enumerate(query_texts):
            ...
            tokens = [
                token for token in tokens if token not in self.STOPTOKENS
            ]
            tagged_queries.append(
                gensim.models.doc2vec.TaggedDocument(tokens, [query_idx]))

        self.model = gensim.models.doc2vec.Doc2Vec(
            vector_size=self.representation_size, min_count=2, epochs=500)
        self.model.build_vocab(tagged_queries)
        logger = logging.getLogger("gensim.models.base_any2vec")
        logger.setLevel(logging.CRITICAL)
        self.model.train(tagged_queries,
...
class SQLWorkloadLSI(WorkloadEmbedder):
    ...
    def __init__(self, query_texts, representation_size, database_connector,
        ...
        self.bow_corpus = [
            self.dictionary.doc2bow(query) for query in self.processed_queries
        ]
...
class PlanEmbedder(WorkloadEmbedder):
    ...
    def __init__(self,
        ...
        WorkloadEmbedder.__init__(self,
                                  query_texts,
                                  representation_size,
                                  database_connector,
                                  columns,
        ...
        self.bow_corpus = [
            self.dictionary.doc2bow(query) for query in self.relevant_operators
        ]
...
class PlanEmbedderPCA(PlanEmbedder):
    ...
    def __init__(self, query_texts, representation_size, database_connector,
                 columns):
        PlanEmbedder.__init__(self, query_texts, representation_size,
                              database_connector, columns)
...
class PlanEmbedderDoc2Vec(PlanEmbedder):
    ...
    def __init__(self,
        ...
        PlanEmbedder.__init__(self, query_texts, representation_size,
                              database_connector, columns, without_indexes)

    def _create_model(self):
        tagged_plans = []
        for plan_idx, operators in enumerate(self.relevant_operators):
            tagged_plans.append(
                gensim.models.doc2vec.TaggedDocument(operators, [plan_idx]))

        self.model = gensim.models.doc2vec.Doc2Vec(
            vector_size=self.representation_size, min_count=2, epochs=1000)
        self.model.build_vocab(tagged_plans)
        logger = logging.getLogger("gensim.models.base_any2vec")
        logger.setLevel(logging.CRITICAL)
        self.model.train(tagged_plans,
                         total_examples=self.model.corpus_count,
                         epochs=self.model.epochs)
        logger.setLevel(logging.INFO)
...
class PlanEmbedderDoc2VecWithoutIndexes(PlanEmbedderDoc2Vec):
    ...
    def __init__(self, query_texts, representation_size, database_connector,
                 columns):
        PlanEmbedderDoc2Vec.__init__(self,
                                     query_texts,
                                     representation_size,
                                     database_connector,
                                     columns,
...
class PlanEmbedderBOW(PlanEmbedder):
    ...
    def _create_model(self):
        assert self.representation_size == len(
            self.dictionary
        ), f"{self.representation_size} == {len(self.dictionary)}"
...
class PlanEmbedderLSIBOWWithoutIndexes(PlanEmbedderLSIBOW):
    ...
    def __init__(self, query_texts, representation_size, database_connector,
                 columns):
        PlanEmbedderLSIBOW.__init__(self,
                                    query_texts,
                                    representation_size,
                                    database_connector,
                                    columns,
