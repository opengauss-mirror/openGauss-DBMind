...
class ConfigurationParser(object):
    ...
    def __init__(self, configuration_file):
        ...
        self.REQUIRED_CONFIGURATION_OPTIONS_FURTHER = {
            ...
            "rl_algorithm": [
                "stable_baselines_version", "algorithm", "gamma", "policy",
                "args"
            ],
        ...
        for key, required_options in self.REQUIRED_CONFIGURATION_OPTIONS_FURTHER.items(
            ...
            self._determine_missing_configuration_options(
                required_options, self.config[key].keys())
    ...
    def _determine_missing_configuration_options(
            ...
            crash_on_fail=True):
        missing_configuration_options = set(
            expected_configuration_options) - set(actual_configuration_options)
    ...
    def _check_dependencies(self):
        if self.config["rl_algorithm"]["algorithm"] == "DQN":
            if self.config["parallel_environments"] > 1:
                raise ValueError(
                    "For DQN parallel parallel_environments must be 1.")
        ...
        if self.config["workload"]["unknown_queries"] > 0:
            assert (sorted(
                ...
                ["unknown_query_probabilities"]) == self.config["workload"]
                    ["validation_testing"]["unknown_query_probabilities"]
        ...
        if ("query_class_change_frequency" in self.config["workload"]
                and self.config["workload"]["query_class_change_frequency"]
                is not None):
            assert (
