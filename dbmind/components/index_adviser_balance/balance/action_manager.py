...
class ActionManager(object):
    ...
    def get_initial_valid_actions(self, workload, budget):
        self.current_action_status = [
            0 for action in range(self.number_of_columns)
        ]

        self.valid_actions = [
            self.FORBIDDEN_ACTION for action in range(self.number_of_actions)
        ]
        self._remaining_valid_actions = []

        self._valid_actions_based_on_workload(workload)
        self._valid_actions_based_on_budget(budget,
                                            current_storage_consumption=0)

        self.current_combinations = set()

        return np.array(self.valid_actions)

    def update_valid_actions(self, last_action, budget,
                             current_storage_consumption):
        assert self.indexable_column_combinations_flat[
            last_action] not in self.current_combinations

        actions_index_width = len(
            self.indexable_column_combinations_flat[last_action])
        if actions_index_width == 1:
            self.current_action_status[last_action] += 1
        else:
            combination_to_be_extended = self.indexable_column_combinations_flat[
                last_action][:-1]
            assert combination_to_be_extended in self.current_combinations

            status_value = 1 / actions_index_width

            last_action_back_column = self.indexable_column_combinations_flat[
                last_action][-1]
            last_action_back_columns_idx = self.column_to_idx[
                last_action_back_column]
            self.current_action_status[
                last_action_back_columns_idx] += status_value

            self.current_combinations.remove(combination_to_be_extended)

        self.current_combinations.add(
            self.indexable_column_combinations_flat[last_action])
    ...
    def _valid_actions_based_on_budget(self, budget,
                                       current_storage_consumption):
        if budget is None:
            return
        else:
            ...
            for action_idx in self._remaining_valid_actions:
                if b_to_mb(
                        current_storage_consumption +
                        self.action_storage_consumptions[action_idx]) > budget:
...
class DRLindaActionManager(ActionManager):
    ...
    def __init__(self, indexable_column_combinations,
                 action_storage_consumptions, sb_version, max_index_width,
                 reenable_indexes):
        ...
        self.indexable_column_combinations_flat = [
            item for sublist in self.indexable_column_combinations
            for item in sublist
        ]
        ...
        self.indexable_columns = list(
            map(lambda one_column_combination: one_column_combination[0],
    ...
    def get_initial_valid_actions(self, workload, budget):
        ...
        self.valid_actions = [
            self.ALLOWED_ACTION for action in range(self.number_of_actions)
        ]
...
class MultiColumnIndexActionManager(ActionManager):
    ...
    def __init__(self, indexable_column_combinations,
        ...
        for idx, column_combination in enumerate(
                self.indexable_column_combinations_flat):
        ...
        for column_combination_idx, indexable_column_combination in enumerate(
            ...
            self.candidate_dependent_map[dependent_of].append(
    ...
    def _valid_actions_based_on_last_action(self, last_action):
        ...
        if last_combination_length != self.MAX_INDEX_WIDTH:
            for column_combination_idx in self.candidate_dependent_map[
                    last_combination]:
                indexable_column_combination = self.indexable_column_combinations_flat[
                    column_combination_idx]
                ...
                self.valid_actions[
                    column_combination_idx] = self.ALLOWED_ACTION

        for column_combination_idx in copy.copy(self._remaining_valid_actions):
            indexable_column_combination = self.indexable_column_combinations_flat[
                column_combination_idx]
            indexable_column_combination_length = len(
                indexable_column_combination)
        ...
        if self.REENABLE_INDEXES and last_combination_length > 1:
            ...
            if len(last_combination_without_extension) > 1:
                last_combination_without_extension_parent = last_combination_without_extension[:
                                                                                               -1]
                if last_combination_without_extension_parent not in self.current_combinations:
                    return

            column_combination_idx = self.column_combination_to_idx[str(
                last_combination_without_extension)]
            self._remaining_valid_actions.append(column_combination_idx)
            self.valid_actions[column_combination_idx] = self.ALLOWED_ACTION

            logging.debug(
                f"REENABLE_INDEXES: {last_combination_without_extension} after {last_combination}"
            )

    def _valid_actions_based_on_workload(self, workload):
        indexable_columns = workload.indexable_columns(return_sorted=False)
        indexable_columns = indexable_columns & frozenset(
        ...
        for indexable_column in indexable_columns:
            for column_combination_idx, indexable_column_combination in enumerate(
                ...
                if indexable_column == indexable_column_combination[0]:
                    self.valid_actions[
                        column_combination_idx] = self.ALLOWED_ACTION
                    self._remaining_valid_actions.append(
                        column_combination_idx)
...
class MultiColumnIndexActionManagerNonMasking(ActionManager):
    ...
    def update_valid_actions(self, last_action, budget,
        ...
        last_action_column_combination = self.indexable_column_combinations_flat[
            last_action]
