...
class WorkloadGenerator(object):
    ...
    def __init__(self,
        ...
        self.globally_indexable_columns = self._select_indexable_columns(
            self.filter_utilized_columns)

        validation_instances = config["validation_testing"][
            "number_of_workloads"]
        ...
        if config["similar_workloads"] and config["unknown_queries"] == 0:
            ...
            _, self.wl_validation[0], self.wl_testing[
                0] = self._generate_workloads(0, validation_instances,
                                              test_instances, config["size"])
            if config["query_class_change_frequency"] is None:
                self.wl_training = self._generate_similar_workloads(
                    config["training_instances"], config["size"])
            else:
                self.wl_training = self._generate_similar_workloads_qccf(
                    config["training_instances"], config["size"],
                    config["query_class_change_frequency"])
            ...
            self.unknown_query_classes = embedding_utils.which_queries_to_remove(
                embedder.plans,
                config["unknown_queries"],
                random_seed,
                experiment_folder_path=experiment_folder_path)

            self.unknown_query_classes = frozenset(
                self.unknown_query_classes) - self.excluded_query_classes
            missing_classes = config["unknown_queries"] - len(
                self.unknown_query_classes)
            self.unknown_query_classes = self.unknown_query_classes | frozenset(
                self.rnd.sample(
                    self.available_query_classes -
                    frozenset(self.unknown_query_classes), missing_classes))
            assert len(self.unknown_query_classes) == config["unknown_queries"]

            self.known_query_classes = self.available_query_classes - frozenset(
                self.unknown_query_classes)
            embedder = None
            ...
            logging.critical(
                f"Global unknown query classes: {sorted(self.unknown_query_classes)}"
            )
            logging.critical(
                f"Global known query classes: {sorted(self.known_query_classes)}"
            )

            for unknown_query_probability in config["validation_testing"][
            ...
            if config["similar_workloads"]:
                ...
                self.wl_training, _, _ = self._generate_workloads(
                    config["training_instances"], 0, 0, config["size"])
            ...
            self.wl_validation = [None]
            self.wl_testing = [None]
            self.wl_training, self.wl_validation[0], self.wl_testing[
                0] = self._generate_workloads(config["training_instances"],
                                              validation_instances,
                                              test_instances, config["size"])

        logging.critical(
            f"Sample training workloads: {self.rnd.sample(self.wl_training, 1)}"
        )
    ...
    def _retrieve_query_texts_random_value(self):
        query_files = [
            open(
                f"{QUERY_PATH}/{self.benchmark}/{self.benchmark}_{file_number}.txt",
                "r")
    ...
    def _set_number_of_query_classes(self):
        if self.benchmark == "TPCH":
            ...
            raise ValueError(
                "Unsupported Benchmark type provided, only TPCH, TPCDS, and JOB supported."
            )
    ...
    def _preprocess_queries(self, queries):
        ...
        for query in queries:
            ...
            if "create view revenue0" in query:
                query = query.replace("revenue0",
                                      f"revenue0_{self.experiment_id}")
    ...
    def _store_indexable_columns(self, query):
        if self.benchmark != "JOB":
            ...
            assert len(
                split) == 2, "Query split for JOB query contains subquery"
    ...
    def _workloads_from_tuples(self, tuples, unknown_query_probability=None):
        ...
        for tupl in tuples:
            ...
            for query_class, frequency in zip(query_classes,
                                              query_class_frequencies):
                ...
                assert len(
                    query.columns
                ) > 0, f"Query columns should have length > 0: {query.text}"

                queries.append(query)

            assert isinstance(
                queries,
                list), f"Queries is not of type list but of {type(queries)}"
            previously_unseen_queries = (round(
                unknown_query_probability *
                len(queries)) if unknown_query_probability != "" else 0)
            workloads.append(
                Workload(
                    queries,
    ...
    def _generate_workloads(self,
        ...
        while required_unique_workloads > len(unique_workload_tuples):
            workload_tuple = self._generate_random_workload(
                size, unknown_query_probability)
            unique_workload_tuples.add(workload_tuple)

        validation_tuples = self.rnd.sample(unique_workload_tuples,
                                            validation_instances)
        unique_workload_tuples = unique_workload_tuples - set(
            validation_tuples)

        test_workload_tuples = self.rnd.sample(unique_workload_tuples,
                                               test_instances)
        unique_workload_tuples = unique_workload_tuples - set(
        ...
        validation_workloads = self._workloads_from_tuples(
            validation_tuples, unknown_query_probability)
        test_workloads = self._workloads_from_tuples(
            test_workload_tuples, unknown_query_probability)
    ...
    def _generate_similar_workloads(self, instances, size):
        ...
        available_query_classes = self.available_query_classes - frozenset(
            query_classes)
        frequencies = list(self.np_rnd.zipf(1.5, size))

        workload_tuples.append(
            (copy.copy(query_classes), copy.copy(frequencies)))
        ...
        for workload_idx in range(instances - 1):
            ...
            query_classes.append(
                self.rnd.sample(available_query_classes, 1)[0])
            frequencies.append(self.np_rnd.zipf(1.5, 1)[0])

            frequencies[self.rnd.randrange(
                len(query_classes))] = self.np_rnd.zipf(1.5, 1)[0]
    ...
    def _generate_similar_workloads_qccf(self, instances, size,
        ...
        while len(workload_tuples) < instances:
            if len(workload_tuples) % query_class_change_frequency == 0:
                query_classes = self.rnd.sample(self.available_query_classes,
                                                size)
    ...
    def _generate_random_workload(self, size, unknown_query_probability=None):
        ...
        if unknown_query_probability is not None:
            ...
            known_query_classes = self.rnd.sample(self.known_query_classes,
                                                  number_of_known_queries)
            unknown_query_classes = self.rnd.sample(self.unknown_query_classes,
                                                    number_of_unknown_queries)
            ...
            if self.gen_one > 0 + 9:
                ...
                workload_query_classes = tuple(
                    self.rnd.sample(self.available_query_classes, size))
        ...
        if self.varying_frequencies:
            query_class_frequencies = tuple(
                list(self.np_rnd.integers(1, 10000, size)))
        else:
    ...
    def _only_utilized_indexes(self, indexable_columns):
        frequencies = [
            1 for frequency in range(len(self.available_query_classes))
        ]
        ...
        utilized_indexes, query_details = get_utilized_indexes(
            workload, candidates, cost_evaluation, True)
        ...
        logging.critical(
            f"Excluding columns based on utilization:\n   {excluded_columns}")
    ...
    def _select_indexable_columns(self, only_utilized_indexes=False):
        ...
        query_class_frequencies = tuple(
            [1 for frequency in range(len(available_query_classes))])

        logging.info(
            f"Selecting indexable columns on {len(available_query_classes)} query classes."
        )

        workload = self._workloads_from_tuples([(available_query_classes,
                                                 query_class_frequencies)])[0]
