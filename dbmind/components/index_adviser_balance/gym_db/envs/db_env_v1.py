...
class DBEnvV1(gym.Env):
    ...
    def __init__(self, environment_type=EnvironmentType.TRAINING, config=None):
        ...
        if self.environment_type != environment_type.TRAINING:
            self.episode_performances = collections.deque(
                maxlen=len(config["workloads"]))
    ...
    def _step_asserts(self, action):
        ...
        assert (self.valid_actions[action] == self.action_manager.
                ALLOWED_ACTION), f"Agent has chosen invalid action: {action}"
        assert (
            Index(self.globally_indexable_columns[action])
            not in self.current_indexes
    ...
    def step(self, action, start=False):
        ...
        savepath = "./indexrank/" + self.config["ids"] + ".pkl"

        startup = False & start
        if (startup):
            if path not in self.dic.keys():
                self.dic[path] = {}
            if path2 not in self.dic[path].keys():
                self.dic[path][path2] = {}

            if action in self.dic[path][path2].keys():
                self.dic[path][path2][action] += 1
            else:
                self.dic[path][path2][action] = 1
        ...
        print_flag = (self.steps_taken
                      >= self.max_steps_per_episode) and testflag
        ...
        current_observation = self.observation_manager.get_observation(
            environment_state, self.config["database_name"])
    ...
    def _init_modifiable_state(self):
        ...
        if self.environment_type == EnvironmentType.TRAINING:
            if self.similar_workloads:
                ...
                self.current_workload = self.workloads.pop(0 +
                                                           self.env_id * 200)
            else:
                self.current_workload = self.rnd.choice(self.workloads)
        else:
            self.current_workload = self.workloads[self.current_workload_idx %
                                                   len(self.workloads)]
        ...
        initial_observation = self.observation_manager.get_observation(
            environment_state, self.config["database_name"])
    ...
    def _update_return_env_state(self,
        ...
        if new_index is not None:
            ...
            if self.current_budget:
                assert b_to_mb(
                    self.current_storage_consumption) <= self.current_budget, (
