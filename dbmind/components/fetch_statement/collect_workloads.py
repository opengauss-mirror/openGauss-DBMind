...
def _add_quote(values, delimiter=','):
    ...
    d = ''
    try:
        d = [f"'{item.strip()}'" for item in values.split(delimiter)]
        d = '(' + ','.join(d) + ')'
    except Exception:
...
def collect_statement_from_statement_history(databases, schemas, start_time, end_time, db_users, sql_types, duration=60):
    ...
    if schemas is not None:
        schemas = _add_quote(schemas)
        stmt += f" and schema_name in {schemas}"
    if sql_types is not None:
        sql_types = _add_quote(sql_types)
        stmt += f" and upper(split_part(trim(query), ' ', 1)) in {sql_types}"
    if db_users is not None:
...
def fetch_statements(args):
    ...
    if args.statement_source == 'asp':
        stmts = collect_statement_from_asp(args.statement_database, args.start_time, args.end_time, args.statement_user, args.statement_type)
    elif args.statement_source == 'pg_stat_activity':
        stmts = collect_statement_from_activity(args.statement_database, args.statement_user, args.statement_type, args.duration)
    elif args.statement_source == 'dbe_perf.statement_history':
        stmts = collect_statement_from_statement_history(args.statement_database, args.statement_schema, args.start_time, args.end_time, args.statement_user, args.statement_type, args.duration)
    if stmts:
...
def main(argv):
    ...
    arg_parser.add_argument("--data-source", choices=('tsdb', 'driver'), default='tsdb', help='set database dsn when tsdb is not available. Using in diagnosis.')
    ...
    arg_parser.add_argument("--statement-database", help="Name of database")
    arg_parser.add_argument("--statement-schema", help="Schema name for the current business data")
    arg_parser.add_argument('--statement-type', help='The type of statementss you want to fetch')
    arg_parser.add_argument('--statement-user', help='The user of statements you want to fetch')
    arg_parser.add_argument('--start-time', type=date_type, help="Start time of fetching statements, "
    ...
    if args.data_source == 'driver':
        ...
        try:
            ...
            if 'password' in parsed_dsn:
                arg_parser.exit(0, "sensitive information")
