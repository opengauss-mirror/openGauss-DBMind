...
class BartForSqlGeneration(BartPretrainedModel):
    base_model_prefix = "model"
    _keys_to_ignore_on_load_missing = [r"final_logits_bias", r"lm_head.weight"]

    def __init__(self, config: BartConfig, model: BartModel):
        super().__init__(config)
        self.model = model
        self.register_buffer("final_logits_bias", torch.zeros((1, self.model.shared.num_embeddings)))
        self.lm_head = nn.Linear(config.d_model, self.model.shared.num_embeddings, bias=False)
    ...
    def load_lm_head(self,GenModel):
        self.lm_head = GenModel.lm_head
    ...
    def forward(
        ...
        masked_index = (input_ids[0] == 50264)
        ...
        lm_logits = self.lm_head(outputs[0])
        lm_logits = lm_logits + self.final_logits_bias.to(lm_logits.device)
        if masked_index >= lm_logits.shape[1]:
            done = 1
        elif labels is not None:
            probs = lm_logits[0, masked_index].softmax(dim=0)
            values, predictions = probs.topk(1)
            if predictions[0] == labels[0,masked_index+1] and done == 0:
                done = 1            
