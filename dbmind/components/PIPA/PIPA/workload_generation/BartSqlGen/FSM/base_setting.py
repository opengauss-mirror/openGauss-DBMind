    ...
    if server_name == 'postgresql':
        ...
        while not sucess and count < 3:
            try:
                ...
                db = psycopg2.connect(database=dbname, user=database["pg_user"], password=database["pg_password"], host=database["pg_ip"], port=database["pg_port"])
...
def get_table_structure(dbname, server='postgresql'):
    ...
    if server == 'mysql':
        ...
        cur_path = os.getcwd()
        tpath = cur_path + '/workload_generation/BartSqlGen/FSM/sampled_data/' + dbname + '/schema'
