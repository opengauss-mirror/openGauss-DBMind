...
QUERY_PATH = "victim_models/SWIRL_2022/query_files"


class WorkloadGenerator_v2(object):
    def __init__(
            self, attack_workload, config, workload_columns, random_seed, database_name, experiment_id=None, filter_utilized_columns=None
    ):
        ...
        self.excluded_query_classes = set(config["excluded_query_classes"])  # tpch 22条query
        ...
        self.query_texts = self._retrieve_query_texts(attack_workload)  # tpch的查询的22条模板
        ...
        self.globally_indexable_columns = self._select_indexable_columns(self.filter_utilized_columns)  # 可索引列
        ...
        elif config["unknown_queries"] > 0:  # 考虑对unseen的query的索引推荐预测
            assert (
    ...
    def set_wl_testing(self, workload):
        self.wl_testing = []
        self.wl_testing.append(workload)
        return self.wl_testing

    def _retrieve_query_texts(self, attack_workload):
        query_files = [
        ...
        for query_file in query_files:
            ...
            query_file.close()
        for workload in attack_workload:
            queries = self._preprocess_queries([workload])
            finished_queries.append(queries)
    ...
    def _generate_probing_workloads(
            self, test_instances, size, unknown_query_probability=None, dic=None
    ):
        # TODO：这个地方得改写成能接收任何query，给出cost的形式
        test_instances = 1
        required_unique_workloads = test_instances
        ...
        for i in test_workloads[0].queries:
            j = test_workloads[0].queries[5]
            flag = 0
            for x in z:
                if x[1] in i.text or x[1].split("_")[1] in i.text:
                    test_workloads[0].queries.remove(i)
                    test_workloads[0].queries.append(j)
                    flag = 1
                    break
            if flag == 0:
                j = i
    ...
    def _generate_probing_workloads_v2(
            self, sql
    ):
        # TODO：这个地方得改写成能接收任何query，给出cost的形式
        queries = []
        if len(sql) > 1:
            for i in range(len(sql)):
                if len(sql[i]) > 1:
                    queries.append(Query(i + 1, sql[i]))
                else:
                    if i != 1 and i != 14 and i != 16 and i != 19:
                        queries.append(Query(i + 1, sql[i][0]))
        else:
            query_sql = Query(1, sql)
            queries.append(query_sql)
        for column in self.workload_columns:
            for i in range(len(queries)):
                if column.name in queries[i].text:
                    queries[i].columns.append(column)

        workload = []
        workload.append(
            Workload(queries, description=f"Contains 0 previously unseen queries.")
        )
    ...
    def _generate_workloads(
        ...
        assert (
                len(train_workload_tuples) + len(test_workload_tuples) + len(
            validation_tuples) == required_unique_workloads
        )
