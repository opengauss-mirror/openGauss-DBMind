...
class DBEnvV1(gym.Env):
    ...
    def step(self, action):
        ...
        for i in self.valid_actions:
            if i == self.action_manager.ALLOWED_ACTION:
                flag = 1
                break
        if flag == 0:
            environment_state = self._update_return_env_state(init=True)
            return self.observation_manager.get_observation(environment_state), 0, True, {"action_mask": self.valid_actions}
