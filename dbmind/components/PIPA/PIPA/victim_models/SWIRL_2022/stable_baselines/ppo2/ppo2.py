...
class PPO2(ActorCriticRLModel):
    ...
    def __init__(self, policy, env, gamma=0.99, n_steps=128, ent_coef=0.01, learning_rate=2.5e-4, vf_coef=0.5,
                 ...
                 full_tensorboard_log=False, seed=None, n_cpu_tf_sess=None, path=None, is_load=False):
        ...
        if _init_setup_model:
            self.setup_model()       
        if is_load:
            self._load_from_file(f"{path}/final_model.zip", load_data=True, custom_objects=None)
        if True:
            a = 1
    ...
    def _train_step(self, learning_rate, cliprange, obs, returns, masks, actions, values, neglogpacs, action_masks, update,
        ...
        if writer is not None:
            ...
            #     tensorflow.summary.scalar("abc",summary, step = (update * update_fac))
            #     writer.flush()
        else:
    ...
    def learn(self, total_timesteps, callback=None, log_interval=1, tb_log_name="PPO2",
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for update in range(1, n_updates + 1):
                ...
                a = copy.deepcopy(self.episode_reward)
                self.reward_all.append(a)
