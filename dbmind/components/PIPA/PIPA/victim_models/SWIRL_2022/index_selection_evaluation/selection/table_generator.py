...
class TableGenerator:
    def __init__(
        ...
        self.columns = []
        self._prepare() 
        if self.database_name() not in self.database_names: # 如果存在想要的table，则不生成了
            self._generate()
            self.create_database()
        else:
            logging.debug("Database with given scale factor already " "existing")
        self._read_column_names() # 读取列名
    ...
    def _prepare(self):
        if self.benchmark_name.split("1")[0] == "tpch":
            ...
            self.directory = "./victim_models/SWIRL_2022/index_selection_evaluation/tpch-kit/dbgen"
            ...
            self.directory = "./victim_models/SWIRL_2022/index_selection_evaluation/tpcds-kit/tools"
