...
class Simulator(BaseSimulator):
    ...
    def run(self, sql):
        ...
        init_cost = pg_client.get_queries_cost([sql, ])[0] * 21
        # init_cost = 0
        ...
        return results, total_time, chosen_arms, init_cost - time_taken
...
if __name__ == "__main__":
    ...
    for r in range(configs.reps):
        simulator = Simulator()
        sql = "select supplier.s_suppkey, partsupp.ps_supplycost from supplier join partsupp on partsupp.ps_suppkey=supplier.s_suppkey where supplier.s_suppkey = 2200 and supplier.s_nationkey = 10;"
        sim_results, total_workload_time, chosen_arms, cost_reduction = simulator.run(sql)
        indexes = ""
