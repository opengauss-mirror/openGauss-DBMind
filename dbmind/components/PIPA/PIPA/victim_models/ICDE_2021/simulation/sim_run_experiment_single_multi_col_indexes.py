...
from simulation.sim_c3ucb_vQ import Simulator
from simulation.sim_ddqn_v3 import Simulator as DDQNSimulator
from simulation.sim_ddqn_v3_single_column import Simulator as DDQNSimulatorSingle

# Define Experiment ID list that we need to run
exp_id_list = ["tpc_h_ddqn_static_3"]
...
for i in range(len(exp_id_list)):
    ...
    DDQNSingle = constants.COMPONENT_DDQN_SINGLE_COLUMN in configs.components
    ...
    if FROM_FILE:
        ...
        if NO_INDEX:
            ...
            for r in range(configs.reps):
                results, total_workload_time = ConfigRunner.run("no_index.sql")
        ...
        if OPTIMAL:
            ...
            for r in range(configs.reps):
                results, total_workload_time = ConfigRunner.run("optimal_config.sql")
                temp = DataFrame(results, columns=[constants.DF_COL_BATCH, constants.DF_COL_MEASURE_NAME,
                                                   constants.DF_COL_MEASURE_VALUE])
                temp.append([-1, constants.MEASURE_TOTAL_WORKLOAD_TIME, total_workload_time])
                temp[constants.DF_COL_REP] = r
        ...
        # Running MAB
        if MAB:
            exp_report_mab = ExpReport(configs.experiment_id, constants.COMPONENT_MAB + exp_id_list[i], configs.reps, configs.rounds)
            for r in range(configs.reps):
                simulator = Simulator()
        ...
        # Running DDQN
        if DDQN:
            exp_report_mab = ExpReport(configs.experiment_id, constants.COMPONENT_DDQN + exp_id_list[i],
                                       configs.reps, configs.rounds)
            for r in range(configs.reps):
        ...
        if DDQNSingle:
            exp_report_mab = ExpReport(configs.experiment_id, constants.COMPONENT_DDQN_SINGLE_COLUMN + exp_id_list[i],
                                       configs.reps, configs.rounds)
            for r in range(configs.reps):
                simulator = DDQNSimulatorSingle()
                results, total_workload_time = simulator.run()
                temp = DataFrame(results, columns=[constants.DF_COL_BATCH, constants.DF_COL_MEASURE_NAME,
                                                   constants.DF_COL_MEASURE_VALUE])
                temp.append([-1, constants.MEASURE_TOTAL_WORKLOAD_TIME, total_workload_time])
        ...
        if SEPARATE_EXPERIMENTS:
            helper.plot_exp_report(configs.experiment_id, exp_report_list, constants.MEASURE_BATCH_TIME, PLOT_LOG_Y)
