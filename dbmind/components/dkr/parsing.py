...
"""This file is used to implement the parsing function of execution plan."""
...
class Operator:
    ...
    def _parse_header(self, line):
        ...
        # Could more details on IndexScan, SeqScan, Sort, etc. From GetPlanNodePlainText().
        self._parse_name()
    ...
    def _parse_name(self):
        if self.name.find('Aggregate') >= 0:
            ...
            if self.name.startswith('Index'):
                if re.findall(IDX_SCAN_PATTERN, self.name):
                    index, tbl = re.findall(IDX_SCAN_PATTERN, self.name)[0]
                if re.findall(IDX_SCAN_PATTERN1, self.name):
                    index, tbl = re.findall(IDX_SCAN_PATTERN1, self.name)[0]
    ...
    def _parse_property(self, line):
        ...
        self.properties[k.strip()] = v.strip()  # more parsing if need
