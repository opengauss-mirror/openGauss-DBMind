...
def get_users(username, password):
    ...
    _res = global_vars.agent_proxy.current_rpc().call_with_another_credential(username, password, 'query_in_postgres',
                                                                              stmt)
    res = []
...
def exec_real_time_inspections(username, password, inspection_type, start_time, end_time, select_metrics):

    if inspection_type not in (
    'real_time_check_daily', 'real_time_check_weekly', 'real_time_check_monthly', 'real_time_check'):
...
def get_current_instance_status():
    ...
    if instance_status['deployment_mode'] == 'centralized':
        ...
        detail['rows'].append(
            [instance_status['primary'], 'primary', True if instance_status['status'] == 'normal' else False])
    return detail
...
def get_collection_system_status():
    collection_detail = {'header': ['component', 'instance', 'listen_address', 'is_alive'], 'rows': [],
                         'suggestions': []}
...
def pg_terminate_pid(username, password, pid):
    ...
    res = global_vars.agent_proxy.current_rpc().call_with_another_credential(username, password, 'query_in_postgres',
                                                                             stmt)
    return psycopg2_dict_jsonify(res)
