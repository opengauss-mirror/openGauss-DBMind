...
export default class KnobData extends Component {
  static propTypes={
    knobData:PropTypes.object.isRequired
  }
  constructor(props) {
    super(props)
    this.state = {
  ...
  async getKnobRecommendation (params) {
    const { success, data, msg } = await getKnobRecommendation(params)
    if (success) {
  ...
  changePage(current,pageSize){
    ...
    this.getKnobRecommendation(params);
  }
    // 回调函数,每页显示多少条
  ...
  UNSAFE_componentWillReceiveProps (nextProps) {
    this.handleTableData(nextProps.knobData.header, nextProps.knobData.rows, nextProps.knobData.total)
  }
  render () {
