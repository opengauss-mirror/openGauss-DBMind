    ...
    Promise.all([
      ...
      if(result[0]){
        ...
        primitiveDataAll.forEach((item,index) => {
                let chartData = []
                let data1 = {'legend':[{image: CurrentReceiveRate, description: 'Current Receive Rate'}],'xAxisData':item[0].timestamps,'seriesData':[{data:item[0].values,description: 'Current Receive Rate', colors: '#2DA769'}],'flg':0,'legendFlg':1,'unit':'KB/s','fixedflg':1}
                let data2 = {'legend':[{image: CurrentSendingRate, description: 'Current Sending Rate'}],'xAxisData':item[1].timestamps,'seriesData':[{data:item[1].values,description: 'Current Sending Rate', colors: '#5990FD'}],'flg':0,'legendFlg':1,'unit':'KB/s','fixedflg':1}
                let data3 = {'legend':[{image: ReceiveDrop, description: 'Receive Drop'},{image: TransmitDrop, description: 'Transmit Drop'}],'xAxisData':item[2].timestamps,'seriesData':[{data:item[2].values,description: 'Receive Drop', colors: '#2DA769'},{data:item[3].values,description: 'Transmit Drop', colors: '#EC6F1A'}],'flg':0,'legendFlg':1,'unit':'','fixedflg':1}
                let data4 = {'legend':[{image: ReceiveError, description: 'Receive Error'},{image: TransmitError, description: 'Transmit Error'}],'xAxisData':item[4].timestamps,'seriesData':[{data:item[4].values,description: 'Receive Error', colors: '#F43146'},{data:item[5].values,description: 'Transmit Error', colors: '#9185F0'}],'flg':0,'legendFlg':1,'unit':'','fixedflg':1}
                ...
                <Panel header={
                  <Row gutter={[0, 12]}>
                    <Col className="gutter-row" span={3}>
                      <span className='networkPanelheader'>{this.state.primitiveDataAll[index][0].labels.device}</span>
                    </Col>
                    <Col className="gutter-row" span={3}>
                    ...
                    <Col className="gutter-row" span={5}>
                      ...
                      <span className='panelTitleBold' >{(this.state.primitiveDataAll[index][0].values[this.state.primitiveDataAll[index][0].values.length-1]*100).toFixed(2)}KB/s</span>
                      <span className='panelCircle circleColorPurple'></span>
                    </Col>
                    <Col className="gutter-row" span={5}>
                      <span className='panelTitleSize'>Current Sending Rate</span>
                      <span className='panelTitleBold' >{(this.state.primitiveDataAll[index][1].values[this.state.primitiveDataAll[index][1].values.length-1]*100).toFixed(2)}KB/s</span>
