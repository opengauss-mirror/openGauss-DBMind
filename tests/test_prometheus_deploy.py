...
def deploy_configs():
    ...
    configs.set('DOWNLOADING', 'host', 'https://127.0.0.1/prometheus')
    ...
    configs.set('PROMETHEUS', 'host', '192.168.0.2')
    ...
    configs.set('EXPORTERS', 'targets',
                '192.168.0.1:19995/postgres, 192.168.0.2:19995/postgres')
...
def test_db_exporters_parsing(deploy_configs):
    ...
    target = {
        '192.168.0.1': defaultdict(
            list,
            {
                'db_instance': ['192.168.0.1:19995/postgres'],
                'opengauss_exporters': ['192.168.0.1:9187']
            }
        ),
        '192.168.0.2': defaultdict(
            list,
            {
                'db_instance': ['192.168.0.2:19995/postgres'],
                'opengauss_exporters': ['192.168.0.2:9187']
            }
        )
    }
    assert (exporters == target)

    configs.set('EXPORTERS', 'targets', '192.168.0.1:19995/postgres, 192.168.0.1:19995/tpcc10')
    ...
    target = {
        '192.168.0.1': defaultdict(
            ...
            {
                'db_instance': [
                    '192.168.0.1:19995/postgres',
                    '192.168.0.1:19995/tpcc10',
                ],
                'opengauss_exporters': [
                    '192.168.0.1:9195',
                    '192.168.0.1:9196',
    ...
    assert (exporters == target)
    configs.set('EXPORTERS', 'targets', '192.168.0.1:19995/postgres, 192.168.0.2:19995/postgres')
    configs.set('EXPORTERS', 'opengauss_ports_range', '9187-9197')
    ...
    target = [
        [
            ...
            [
                ...
                '--config.file /media/sdb/cent/deploy/prometheus-2.35.0-rc0.linux-amd64/prometheus.yml '
                '--web.enable-admin-api '
                '--storage.tsdb.retention.time=1w '
                '>/media/sdb/cent/deploy/prometheus.log',
                '',
                'Prometheus of 192.168.0.2'
            ],
            [
                '/media/sdb/cent/deploy/gs_dbmind component '
                'reprocessing_exporter 192.168.0.2 9090 '
                '--web.listen-address 0.0.0.0 '
                '--web.listen-port 8181 '
                '--disable-https',
                'reprocessing-exporter of 192.168.0.2 has been started or the address already in use.',
                'reprocessing-exporter of 192.168.0.2'
            ],
            'chmod 600 /media/sdb/cent/deploy/prometheus.log',
            ...
            [
                ...
                '',
                'node-exporter of 192.168.0.1'
            ],
            [
                '/media/sdb/deploy/gs_dbmind component cmd_exporter '
                '--web.listen-address 0.0.0.0 '
                '--web.listen-port 9180 --disable-https',
                'cmd-exporter of 192.168.0.1 has been started or the address already in use.',
                'cmd-exporter of 192.168.0.1'
            ],
            [
                '/media/sdb/deploy/gs_dbmind component opengauss_exporter '
                '--dsn \'dbname=postgres host=192.168.0.1 port=19995 user=dbmind password=3\' '
                '--web.listen-address 0.0.0.0 '
                '--web.listen-port 9187 '
                '--disable-https',
                'opengauss-exporter of 192.168.0.1:9187 has been started or the address already in use.',
                'opengauss-exporter of 192.168.0.1:9187'
            ],
            'chmod +x /media/sdb/deploy/node_exporter-1.3.1.linux-amd64/node_exporter',
            [
                '/media/sdb/deploy/node_exporter-1.3.1.linux-amd64/node_exporter '
            ...
            [
                ...
                '--disable-https',
                'cmd-exporter of 192.168.0.2 has been started or the address already in use.',
                'cmd-exporter of 192.168.0.2'
            ],
            [
                '/media/sdb/deploy/gs_dbmind component opengauss_exporter '
                '--dsn \'dbname=postgres host=192.168.0.2 port=19995 user=dbmind password=3\' '
                '--web.listen-address 0.0.0.0 '
                '--web.listen-port 9187 '
                '--disable-https',
                'opengauss-exporter of 192.168.0.2:9187 has been started or the address already in use.',
                'opengauss-exporter of 192.168.0.2:9187'
            ]
        ]
    ]
    mock_PWD = {
...
def test_generate_checks(deploy_configs):
    target = [
        {
            'url': '192.168.0.2:9090/api/v1/query?query=up',
            'type': 'prometheus-server',
            'address': '192.168.0.2:9090',
            'listen': '0.0.0.0:9090',
            'target': '-',
            'status': 'Down'
        },
        {
            'url': '192.168.0.2:8181/metrics',
            'type': 'reprocessing-exporter',
            'address': '192.168.0.2:8181',
            'listen': '0.0.0.0:8181',
            'target': '192.168.0.2:9090',
            'status': 'Down'
        },
        {
            'url': '192.168.0.1:9100/metrics',
            'type': 'node-exporter',
            'address': '192.168.0.1:9100',
            'listen': '0.0.0.0:9100',
            'target': '192.168.0.1',
            'status': 'Down'
        },
        {
            'url': '192.168.0.1:9180/metrics',
            'type': 'cmd-exporter',
            'address': '192.168.0.1:9180',
            'listen': '0.0.0.0:9180',
            'target': '192.168.0.1',
            'status': 'Down'
        },
        {
            'url': '192.168.0.1:9187/metrics',
            'type': 'opengauss-exporter',
            'address': '192.168.0.1:9187',
            'listen': '0.0.0.0:9187',
            'target': '192.168.0.1:19995/postgres',
            'status': 'Down'
        },
        {
            'url': '192.168.0.2:9100/metrics',
            'type': 'node-exporter',
            'address': '192.168.0.2:9100',
            'listen': '0.0.0.0:9100',
            'target': '192.168.0.2',
            'status': 'Down'
        },
        {
            'url': '192.168.0.2:9180/metrics',
            'type': 'cmd-exporter',
            'address': '192.168.0.2:9180',
            'listen': '0.0.0.0:9180',
            'target': '192.168.0.2',
            'status': 'Down'
        },
        {
            'url': '192.168.0.2:9187/metrics',
            'type': 'opengauss-exporter',
            'address': '192.168.0.2:9187',
            'listen': '0.0.0.0:9187',
            'target': '192.168.0.2:19995/postgres',
            'status': 'Down'
        }
    ]
    configs = deploy_configs
...
def test_check_config_validity():
    ...
    section, option, value = "PROMETHEUS", "listen_address", "0192.168.0.1"
    assert (check_config_validity(section, option, value) ==
            (False, 'Invalid IP Address for PROMETHEUS-listen_address: 0192.168.0.1'))
    ...
    section, option, value = "EXPORTERS", "targets", "192.168.0.1:19995/postgres, "
    assert (check_config_validity(section, option, value) ==
            (False, 'Illegal db instance " ", e.g. ip:port/dbname'))

    section, option, value = "EXPORTERS", "targets", "192.168.0.1:19995:postgres, 192.168.0.2:19995/postgres"
    assert (check_config_validity(section, option, value) ==
            (False, 'Illegal db instance "192.168.0.1:19995:postgres", e.g. ip:port/dbname'))

    section, option, value = "EXPORTERS", "targets", "192.168.0.1:19995/postgres, 192.168.0.2/19995/postgres"
    assert (check_config_validity(section, option, value) ==
            (False, 'Illegal db instance " 192.168.0.2/19995/postgres", e.g. ip:port/dbname'))

    section, option, value = "EXPORTERS", "targets", "192.168.0.1:1023/postgres, 192.168.0.2:19995/postgres"
    assert (check_config_validity(section, option, value) ==
            (False, 'Invalid port "1023" (1024-65535) for "192.168.0.1:1023/postgres" in EXPORTERS-targets.'))

    section, option, value = "EXPORTERS", "targets", "192.168.0.1:19995/postgres:, 192.168.0.2:19995/postgres"
    assert (check_config_validity(section, option, value) ==
            (False, 'Illegal db instance "192.168.0.1:19995/postgres:", e.g. ip:port/dbname'))

    section, option, value = "EXPORTERS", "targets", "192.168.0.1:19995/ , 192.168.0.2:19995/postgres"
    assert (check_config_validity(section, option, value) ==
            (False, 'Empty dbname " " for "192.168.0.1:19995/ " in EXPORTERS-targets.'))

    section, option, value = "EXPORTERS", "targets", "192.168.0.1:19995/postgres, 192.168.0.2222:65536/postgres"
    assert (check_config_validity(section, option, value) ==
            (False, 'Invalid IP " 192.168.0.2222" and Invalid port "65536" (1024-65535) '
                    'for " 192.168.0.2222:65536/postgres" in EXPORTERS-targets.'))
