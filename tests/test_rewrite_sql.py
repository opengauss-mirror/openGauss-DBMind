...
WHERE s_i_id IN (1,
  ...
  OR s_w_id IN (10,
    ...
    'In2Exists': {
        ...
        'SELECT * FROM T1 WHERE T1.C1 NOT IN (SELECT T2.C2 FROM T2 WHERE T2 IN (30, 40));':
            'SELECT * FROM t1 WHERE t1.c1 NOT IN (SELECT t2.c2 FROM t2 WHERE t2 IN (30, 40));',
        'SELECT * FROM T1 WHERE T1.C1 IN (SELECT T2.C2 FROM T2);':
            'SELECT * FROM t1 WHERE EXISTS (SELECT * FROM t2 WHERE t1.c1 = t2.c2);',
        'SELECT * FROM T1 WHERE T1.C1 IN (SELECT T2.C2 FROM T2) AND C1 IN (30, 40);':
            'SELECT * FROM t1 WHERE EXISTS (SELECT * FROM t2 WHERE t1.c1 = t2.c2) AND c1 IN (30, 40);',
